...
| `agent.mandate-exceeded` | The amount is over the maximum single bid, or over the daily cap. The body carries `limit` and `requested`, and `used` for the daily cap only. | Lower the amount, or ask the human for a larger mandate. |
...
| `agent.idempotency-key-required` | A bid call carried no key, or a malformed key. | Repeat the call with an `idempotency_key` of 1 to 255 visible ASCII characters. |
